...
import com.overstreamapp.x32mixer.X32MixerSettings;
...
public class DefaultX32MixerClient implements X32MixerClient {
    ...
    @Override
    public void channelOn(int channel, boolean on) {
        if (channel >= 1 && channel <= 32) {
            mixer.channelOn(channel, on);
        }
    }

    @Override
    public void busOn(int bus, boolean on) {
        if (bus >= 1 && bus <= 16) {
            mixer.busOn(bus, on);
        }
    }
