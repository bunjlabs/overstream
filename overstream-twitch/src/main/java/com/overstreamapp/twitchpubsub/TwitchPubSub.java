...
public class TwitchPubSub {
    ...
    @Inject
    public TwitchPubSub(Logger logger, TwitchPubSubSettings settings, WebSocketClient webSocketClient, EventLoopGroupManager loopGroupManager, EventKeeper eventKeeper) {
        this.logger = logger;
    ...
    private class Handler implements WebSocketHandler {
        ...
        public void onOpen(WebSocket socket) {
            TwitchPubSub.this.webSocket = socket;
            TwitchPubSub.this.state = ConnectionState.CONNECTED;

            TwitchPubSub.this.pingScheduledFuture
                    = TwitchPubSub.this.loopGroupManager.getWorkerEventLoopGroup().scheduleAtFixedRate(
                    TwitchPubSub.this::ping, 0, 1, TimeUnit.MINUTES);

            subscribedTopics.forEach(TwitchPubSub.this::send);
            ...
            try {
                ...
                switch (message.getType()) {
                    ...
                    case PONG:
                        ...
                        break;
                    case MESSAGE:
                        TwitchPubSub.this.onMessage(message);
                        break;
                }
            } catch (IOException e) {
